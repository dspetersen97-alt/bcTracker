...
render_env() {
    ...
    scheme_host="https://$host"
    {
        ...
        # The whole of a LAN install's difference to the proxy, and deliberately a
        # value in .env rather than an edited copy of the Caddyfile: the copy this
        # replaces was written once, at install, and every security header changed
        # afterwards stopped at the tracked file nothing mounted.
        if [ "$internal_tls" = "yes" ]; then
...
# Only --data-dir needs one now. A LAN install used to be here too, mounting a
# copy of the Caddyfile with `tls internal` added; it sets CADDY_TLS in .env
# instead, so the proxy configuration an upgrade ships is the one it serves.
if [ -n "$data_dir" ]; then
